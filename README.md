...
[disaster-alert](https://github.com/luyi2008/disaster-alert) 的订阅页和通知详情页前端。本站只订阅和展示**地震预警**与**地震速报**。服务端若仍返回气象预警、海啸预警或台风信息，页面会忽略这些类别，保存订阅时也不会写回。本仓库只包含前端代码，不运行灾害数据源、不做 Bark 推送，也不存设备/账号凭据；服务端完全独立发版，两边不需要对齐 git tag。
