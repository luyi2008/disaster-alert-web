...
没有引入状态管理库。交互控件走 `src/components/ui/`（Button、Dialog、AlertDialog、Input、DropdownMenu 等），主题变量接到现有 `tokens.css`（`--primary: #2563eb`、light/dark 跟 `prefers-color-scheme`，不引入 `class="dark"` 开关）。页面级布局仍用手写 CSS（`ds.css` 的登录/壳层、`subscribe.css` 双栏、`detail.css` 时间线、`test.css` 规则卡），地图继续 Leaflet。
