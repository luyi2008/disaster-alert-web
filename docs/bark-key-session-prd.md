...
- 已保存订阅从服务端读取；未提交的地点和规则只保存在页面内存中。登出不删除服务端订阅。

### 非目标

- 不引入密码、邮箱或其它账号体系。
- 不在 localStorage 保存订阅草稿，也不主动删除浏览器里可能遗留的旧草稿键。
...
订阅页通过 Bearer `GET /api/subscriptions` 读取已保存配置，未提交编辑只在页面内存中存在。旧版本可能留下的草稿键会被忽略，不会被读取或删除。
