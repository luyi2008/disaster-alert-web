...
首页 `/` 的工作区仍是命令式 DOM（从原静态页迁入），不是 React 组件树。React 只包一层：`SubscribePage` 负责责任声明、把 `shell.html` 塞进宿主节点、调用 `mountSubscribeApp`，并在卸载时执行返回的 teardown。

通知详情页 `/incidents/...` 已是 React，不要把两边的拆分策略混用。
