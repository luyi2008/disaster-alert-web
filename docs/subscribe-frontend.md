...
- 登录身份是 BFF cookie。订阅读写都走 `/api/subscription/:device_key/subscription`（读）与 `/api/subscription/:device_key/subscribe`（写，BFF 转发到主 API）。401 回 `/login`，设备 404 回 `/devices`。
