...
生产：浏览器直连 `CAPTCHA_ORIGIN`（构建时变量，见 `.env.production`）下的 `/api/captcha` 与 `/api/captcha/verify`，不经站点反代。这是跨域请求，且 `credentials: "include"`，因此 ESA 必须对站点 Origin 返回明确的 `Access-Control-Allow-Origin`（不能 `*`）和 `Access-Control-Allow-Credentials`。

开发：不设 `CAPTCHA_ORIGIN`，请求仍是同源 `/api/captcha*`。本仓库 Vite 必须把这几条写在通配 `/api` → Rust **之前**：
...
| 文件 | 职责 |
| --- | --- |
| `src/auth/captcha.ts` | `captchaUrl()` 拼接 `CAPTCHA_ORIGIN`；`GET /api/captcha`、`POST /api/captcha/verify`；解析 Token 与 SVG；类型里没有正确答案 |
...
3. **disaster-alert-web**：弹层打 `/api/captcha`、`/api/captcha/verify`；生产 `CAPTCHA_ORIGIN` 直连 ESA，开发 Vite 同源代理；更新 `openapi.yaml`。
