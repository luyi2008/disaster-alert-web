...
describe("BFF device client", () => {
  ...
  it("GETs /api/subscription/:device_key/subscription without Authorization", async () => {
    const fetchMock = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => new Response(
    ...
    expect(String(fetchMock.mock.calls[0]![0])).toContain(`/api/subscription/${DEVICE.deviceKey}/subscription`);
